...
| 30 | chapter3/log-sanitization 复刻(平行窗口;顺手起 Ollama 为路线 4 热身) | ✅ 已完成(2026-09-01;平行窗口 11 阶段全完工,产物 `日志脱敏复刻/`(lessons 0001-0011+learning-records 同步+pytest 7 绿);收官交付物 `日志脱敏复刻/对照复盘-三引擎分工拍板.md`:四方同卷成绩单(regex 泄露8/0.1ms,LLM 泄露10,hybrid 泄露2/R0.94,Presidio 泄露11)+五维必答+三出口拍板(memory.json=Presidio 保留;trace/本地日志=regex 在线全量+hybrid 离线补扫),阶段 33 可引用;重要实测:阶段 9 推翻"零命中才进LLM"省成本设计(混合样本漏脱住址),揪出占位符幻觉磁铁失效模式;Ollama 0.33.0+qwen3:0.6b 已装为路线 4 复用(按用户偏好手动档不自启)) |
...
| 33 | 收官:`deliverables/route2/` 三件交付物;票 09 写 Answer 关闭 | ✅ 已完成(2026-09-01;三件:01-边界对比报告(七连探针+双后端灯表+光谱理论,判决一次性 microVM)/02-劫持无效化验证记录(四攻击全过+防线命中统计+发现与残余)/03-缺口1核销记录(两层防御+凭证走私封死,route1 缺口清单已回写);票 09 Answer 六条偏差+残余归属,Status=closed;map.md 已决索引收编路线 2 执行行;双复刻收官拍板均被交付物引用) |
