...
这不是教学练习，是能拿去汇报的小型产品。反玩具标准：① 真实数据（Wazuh 格式告警/TheHive 案件模型）② 真实攻击（路线 1-3 实测语料 + AgentDojo/garak）③ 实证数字 ④ 机制完整（令牌真签真验真焚）⑤ 一键可起 + README 即讲解稿 ⑥ 决策有据（参照系或实证）。**规模可小，每个组件必须是真的；宁砍组件，不降真实度。**
...
2. **制造需求**（grilling）：结合 目标能力要求（三份目标画像，见 立项调研.md（已脱敏）），把" SOC 数字员工"展开成需求集——用户角色、使用场景、功能列表（每个功能标注：服务哪个 目标场景 能力点/哪张亮点清单）
