...
2. **排期方式：滚动式（rolling wave）**——只详细排当前路线；每关收官时根据该关缺口清单详细安排下一关。主干顺序锁定：路线 2 → 路线 3 → 路线 4 红队部分 → "熟悉"档收尾（端云隐私 + 推理端，能讲即可）+ 汇报材料整合 + 零信任架构蓝图。粗略量级参照（路线 1 实际 2 天）：路线 2 约 3–4 天，路线 3 约 4–5 天（最大工程关，含缺口 2/3/7 核销），路线 4 约 3–4 天，收尾 2–3 天——仅作参照，不作死线。
3. **路线 3 范围**：照单全收——缺口 2/3/7（记忆装载校验、执行串联闸、毒源合并处理）的核销归入路线 3 验收项；Presidio 脱敏管道压缩为 1 天 hands-on（无真实 PII 场景，但 目标场景 第 3 条要求保留实操）。
