...
- 价值：① 629 条 injection payload 是现成的回归集语料，可挑与"工具滥用/数据外泄"相关的条目移植进 PyRIT 回归集；② 它的三指标（benign utility / utility under attack / security）是汇报可讲的评估方法论；③ 想了解防御侧（tool_filter 等）实现时可读源码。
