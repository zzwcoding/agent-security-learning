...
> 事实底座：`立项调研.md（已脱敏）` + 六份参照深析（`ref-tracecat.md` / `ref-agentic-soc-platform.md` / `ref-m507-ai-soc-agent.md` / `ref-thehive-cortex.md` / `ref-wazuh-alerts.md` / `ref-holmesgpt.md`）
...
> 每个功能标注：**目标场景 对位**（服务哪个 目标场景 能力点/哪张亮点清单）与**参照**（来源项目）。目标场景 编号见 research 底座：目标场景#1=安全公司 Agent 架构师、目标场景#3=下一代 AI Agent 系统（最优目标）。
...
- 目标场景 对位：目标场景#3「Tool 调用体系（检索/环境操作）」的输入侧；目标场景#1 主营产品语境（WAF/日志审计的告警长这样）｜参照：Wazuh 报告 §6 映射表、TheHive 去重机制
...
- 目标场景 对位：目标场景#3「Memory 机制/状态管理」｜参照：TheHive 报告 §2/§3
...
- 目标场景 对位：**两份对口 目标场景 共同第一缺口**——目标场景#3「多 Agent 协同架构/端到端任务执行流程」、目标场景#1「多 Agent 协作模式」｜参照：agentic-soc-platform 分工、M507 tier 模型
...
- 目标场景 对位：目标场景#3「任务理解与规划拆解」｜参照：M507 runbook 格式改造为节点 prompt 契约
...
- 目标场景 对位：目标场景#3「上下文管理与 Token 效率优化」「多轮推理链路设计」｜参照：HolmesGPT §2
...
- 目标场景 对位：目标场景#1 工具编排｜参照：Cortex analyzer 契约
...
- 目标场景 对位：目标场景#3「自我迭代能力的智能体系统」**（注意：这是知识沉淀不是 prompt 自我迭代，后者已砍）**；目标场景#1 RAG 硬性要求｜参照：HolmesGPT §4、M507 §2
...
- 亮点清单对位：目标场景#1 加分项「密钥安全存储」｜参照：Tracecat §5、HolmesGPT §6
...
- 亮点清单对位：目标场景#1「RBAC 权限管控」｜参照：HolmesGPT §6、M507 §5、Tracecat 审批
...
- 亮点清单对位：目标场景#1「Prompt 注入防护」｜参照：Wazuh 报告 §4 注入面表
...
- 目标场景 对位：目标场景#1 加分项「MCP 协议实践」、腾讯天御同类卖点｜参照：research 底座 §二
...
- 架构叙事：网关注入层可插拔渠道 = 目标场景#1「多渠道网关」的对位
...
- 技术：vitest（非 pytest）
- 目标场景 对位：**评测体系是我们从缺口改记的强项**——目标场景#3「Agent 评测与评估体系（Eval/Harness/自动化评测）」｜参照：HolmesGPT §5

---

## 4. 演示脚本草案（汇报剧本）

> 目标：15 分钟演完全部亮点清单。每一幕对应可复测的 fixture。
