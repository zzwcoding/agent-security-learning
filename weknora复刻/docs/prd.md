...
> 版本：v0.1（2026-09-10，L1 设计窗口起草；11 维逼问 + Web/Agent 加问全维标注完毕）→ v1.0（2026-09-10，主窗口收口：范围含/不含逐条确认 + B 类问题 1 拍板（智谱 GLM：embedding-3 + glm-4-flash），全文待确认项清零，翻「已定稿」）。
...
| 向量检索 | numpy 暴力全扫余弦，3000 × 2048 维（embedding-3 默认维）float32 ≈ 25MB 内存，单次查询 <100ms | 不需要 ANN，暴力扫即教学正确版 |
...
| ⑧ 输入输出与外部集成 | 适用 | 输入=txt/md/pdf 文件；外部集成=一家 LLM/Embedding 真实 API（经统一 gateway）+ fake stub；失败处理=gateway 统一错误，CI 走 stub 不依赖网络 |
...
5. **LLM/Embedding 供应商**：智谱 GLM——embedding 用 `embedding-3`（默认 2048 维，可调 256/512/1024/2048），对话用 `glm-4-flash`（免费档）；key 已在 Keychain（`agent-key glm`）。embedding-3 按 token 计费，但本项目总量 150 万 token 内、总成本约一两元（§5）。备选过的本地方案（Ollama bge-m3 嵌入 + GLM 对话）未选：多一个本地服务要起，教学价值与联网 API 等价；换模型只改 gateway 配置，但换 embedding 模型须重建向量索引（INV-2）。
