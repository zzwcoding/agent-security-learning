...
**What to build:** 学习者跟场景 1 文档+大图学 1.1-1.4 的卡点复盘（2026-09-11，47 问归类两轮）已沉淀为机制：learn-by-rebuild"叙述完备性七原则"、soc-diagram-conv"图文对齐约定+大图职责分工"、`lessons/scenario/TERMS.md` 术语表（含分层与常见误读）。本票把**存量产物**按新机制增补对齐。范围与优先级：

0. **P0 · 0-0 全景导览（新增 `lessons/scenario/0-0.md`，地图先行——七原则 6）**：编制表（9 容器一行一个：演什么角色/技术栈/端口）、常驻循环表（autorun 2s / 派活 100ms / SSE 100ms——各盯哪张表、谁接手）、身份切换图（一条告警流转中 run 的工种怎么切换、每段拿什么票）、全局数字（9 容器/3 循环/14 安全节点/2 张 outbox 屏）、与大图的分工声明（入门靠本篇、回查靠大图）。1.1 开讲前先读它。
1. **P1 · 1-1..1-4 增补（1.3 站 3 做"试点"先行，学习者窗口已提议）**：
