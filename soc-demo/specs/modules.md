...
> 2026-09-13 票 92 补卡：m14 公开接口增 `GET /api/v1/templates`（模板登记面的只读投影）——上行「无独立 HTTP 面」措辞同步修正为卡头「无独立写面（只读模板清单 GET 除外）」。
...
- 过载卸载（票 68，2026-09-13 收口）：`src/under-pressure.ts`——`UNDER_PRESSURE=on` 才 register `@fastify/under-pressure`（env 缺省=零注册=默认形态逐字节不变）；阈值 `UNDER_PRESSURE_MAX_*` env 可覆盖；503 语义插件自带，setErrorHandler 仅放行 `FST_UNDER_PRESSURE`；on 时 `/status` 为插件自带指标口
- 事件等待型 run 的分发放行（票 73，2026-09-13 L0 追认）：run-dispatcher 对注册表 `parksOnEvents` 标记的 kind（现仅 hunt_flow）的 start 任务「领了就放」——串行循环若被 await_children 挂起的 execute 顶住，扇出的子 run 永远领不到任务（生产死锁，probe 实证）；真完成句柄脱账自理（失败由执行件落 run failed+审计+error 事件，重启孤儿收口 `recoverDispatcherState` 照旧），dispatchOnce 契约与既有分发测试零改动。此即"循环拓扑落 dispatcher 层"的机制落点
...
- 七个路由（告警列表/流水线视图/审批卡/案件时间线/审计流/Eval 结果/狩猎页），之外无任何路由（狩猎页=票 82 落地，页面映射节第七行）
...
**M14 编排循环（假设驱动）** → services/agent 内子模块（m3 领地新目录，无独立写面——只读模板清单 GET 除外，票 92；PRD §13）
...
- `GET /api/v1/templates` → 模板登记面的公开只读投影（已登记模板数组：template_id/假设句式族/菜单子集/轮次上限，字段名照模板文件原样；未登记 = 空数组不报错）。只读 L0 面、零业务逻辑（R10：清单数据不是狩猎业务逻辑，数据从登记面读出即返回）；鉴权口径与 agent 既有公开读面（GET /api/v1/approvals）一致——狩猎页模板下拉数据源（票 92 补卡）
...
| 狩猎页·发起假设（选模板+填假设句） | 写+读 | m2 `POST /api/v1/hypotheses` → outbox 拉起 m3 hunt_flow；模板下拉数据源 = m14 `GET /api/v1/templates` 只读投影（票 92 补面） | 需要新接口（票 73）；下拉面票 92 |
