# SOC 数字员工 · 产品需求文档（PRD v1.2）

状态: 已定稿

> 归位说明（2026-09-08，票 18 收口）：本文件自本日起归位为 soc-demo 仓库 `docs/prd.md`，是 sdd-flow 产物契约规定的 PRD 唯一真源；`deliverables/route5/product-handbook.md` 原件顶部已加指向说明、不再更新。

> 路线 5 需求层文档。v0.1 为票 17「路线 5 阶段 A：参考项目深析与需求制造」产出的草案；v0.2 在完整保留 v0.1 六项冻结内容（§0 水位线 / §1 叙事 / §10 参照对照表 / §11 边界声明 / §12 决策记录）的基础上，扩写到可指导开发施工的细度；v1.0 经用户过审、12 条待定项全部拍板后冻结；v1.1 追加三条冻结后范围变更（Langfuse 降可选 / microsandbox 改挂 M6 / M6 升级半真沙箱模块，见文末变更记录）。
> 状态：**v1.1，冻结 + 变更记录（2026-09-04）**，作为阶段 B（票 18 技术设计）的输入。
...
这不是教学练习，是能拿去汇报的小型产品。全程约束（继承票 17 水位线六条）：
...
> 每模块含：职责与边界 / 功能点表（FR-Mx.y，P0=演示必备，P1=加分）/ 实现机制 / 接口契约 / 异常与边界 / 验收标准 / 目标场景 对位与参照。工具清单的完整分级见附录 A.1。
...
**目标场景 对位**：目标场景#3「Tool 调用体系（检索/环境操作）」的输入侧；目标场景#1 主营产品语境｜参照：Wazuh 报告 §6 映射表、TheHive 去重机制。
...
**目标场景 对位**：目标场景#3「Memory 机制/状态管理」｜参照：TheHive 报告 §2/§3/§6、ASP 状态机与双轨 verdict。
...
**目标场景 对位**：**两份对口 目标场景 共同第一缺口**——目标场景#3「多 Agent 协同架构/端到端任务执行流程」、目标场景#1「多 Agent 协作模式」｜参照：agentic-soc-platform 共享资源层分工、M507 routing_rules、Tracecat 资源兜底与审批持久性语义、HolmesGPT 防打转守卫（`prevent_overly_repeated_tool_call`，同参数重复调用直接报错）。
...
**目标场景 对位**：目标场景#3「任务理解与规划拆解」｜参照：M507 runbook 格式/verdict 生命周期/同主机 24h 归并/自我审计 checkpoint、client_env KB 优先策略。
...
**目标场景 对位**：目标场景#3「上下文管理与 Token 效率优化」「多轮推理链路设计」｜参照：HolmesGPT §2 上下文工程、M507 SIEM pivot、TheHive task log。
...
**目标场景 对位**：目标场景#1 工具编排｜参照：Cortex analyzer 契约与 max_tlp/max_pap 闸门、flavor 机制（本 demo 只实现 GetReport 只读 flavor）。
...
**目标场景 对位**：目标场景#3「自我迭代能力的智能体系统」（知识沉淀≠自我迭代，人审入库是硬闸）；目标场景#1 RAG 硬性要求｜参照：HolmesGPT §4 skill 闭环、M507 §2 fine-tuning 推荐闭环、ASP 无判定不抽取门控。
...
**目标场景 对位**：目标场景#1「多渠道网关」（网关注入层可插拔渠道叙事）、目标场景#3 对话式交互｜参照：Tracecat 三态裁决、ASP field-profile、HolmesGPT 验签防伪造。

---

### M9 安全控制面（差异化主体）

> 设计哲学一句话（Tracecat 官方威胁模型表述，汇报最值钱的引句）：**LLM 是不可信的决策者——它不能给自己授权、不能批准自己的调用、不能解析凭证、不能改策略**〔ref-tracecat §4〕。以下 S1-S6 全部 fail-closed。
...
**目标场景 对位**：目标场景#1 加分项「密钥安全存储」｜参照：Tracecat §4/§5、HolmesGPT §6。
...
**目标场景 对位**：目标场景#1「RBAC 权限管控」｜参照：HolmesGPT §6 mint/verify token 与参数级审批、M507 §5 decision_authority、Tracecat 审批持久性语义与三态裁决。
...
**目标场景 对位**：目标场景#1「Prompt 注入防护」｜参照：Wazuh 报告 §4 注入面表、六家全无的对照事实（§10）。
...
**目标场景 对位**：目标场景#1 私有化部署合规叙事｜参照：Tracecat 架构文档的空白声明。
...
**目标场景 对位**：目标场景#1 审计合规｜参照：TheHive Audit schema、Tracecat 审计事件模型。

#### S6 子 agent 权限收窄（亮点清单）
...
**验收标准**：遍历测试：每个 worker × 每个非其 scope 工具 = 100% 403（eval 矩阵断言）；汇报演示点：「分诊 agent 物理上没有遏制工具可调用——验票闸里没有它的票」。

**目标场景 对位**：目标场景#1 RBAC + 目标场景#3 多 agent 安全的交叉亮点清单｜参照：research 底座 §五（子 agent 权限收窄并入 demo 验收）。
...
**目标场景 对位**：目标场景#1「多渠道网关」（网关注入层可插拔渠道）｜参照：已拍板决策 #6（范围锁死六页面）。
...
**目标场景 对位**：**评测体系是从缺口改记的强项**——目标场景#3「Agent 评测与评估体系（Eval/Harness/自动化评测）」｜参照：HolmesGPT §5 全套（fixture 制/judge/断言/CI 分层/ITERATIONS）、M507 成本 CSV、ASP AI-Human Agreement 口径。
...
**目标场景 对位**：目标场景#1 加分项「MCP 协议实践」、腾讯天御「MCP 体检+运行时防护」同类卖点｜参照：research 底座 §二/§五（0038 产品化）。
...
| RAG | —（SOC 参照系均无） | 全部 | 目标场景#1 硬性要求 + 投毒演示 |
...
## 13. 假设驱动编排循环（2026-09-12 变更触发，票 70，逼问中）

> 变更范围声明：本章为 v1.1 冻结后新增能力。§0-§12 冻结内容除本章明确联动的行（演示剧本幕 7、附录 A.1 工具表、M3/M9 卡的联动句）外一律不动。逼问仅重开受影响维度，逐维结论登记 §13.5。

### 13.1 交付物分层（本章灵魂）

**交付物是循环机制，不是狩猎业务。** 任何业务的未来增量不得触及 m14 循环卡——分层拍板清单：

| 层 | 内容 | 形态 |
|---|---|---|
| 机制层（m14） | planner/judge/gap_analyzer 三 LLM 节点、轮次链、扇出/收敛、两票票务、预算双闸 | 代码，写一次 |
| 内容层（业务） | 假设模板 = 句式族 + 菜单子集 + 轮次上限 | 数据（模板文件/剧本库），每新业务零机制增量 |
| 验证层 | 应急取证模板作为"零 m14 增量"架构验收实验（票 80） | 断言：diff 与机制层交集为空 |

### 13.2 机制层水位线（已拍板，A 类补全）

planner 只许从能力菜单选组合（未登记 fail-closed）；LLM 输出全走 schema 校验 + 降级；上游 LLM 产物（子报告/调查报告）进 prompt 前过注入扫描；子票 scope ⊆ 父票菜单；judge/gap/planner 均不持 L2 通道（遏制建议止于文本，动作走人工审批）；全链路审计五要素 + SSE 可回放；预算双闸（run 级 + 轮次级 + 轮次防转指纹）；跨 run 等待走事件唤醒禁轮询。[以上 AI自动补全，沿用 §7/§9 既有纪律，阶段 2 设计确认时复核]

### 13.3 首批业务（已拍板）

- **威胁狩猎**（首发载体）：分析师假设 → 循环验证/证伪 → 结论沉淀。
- **应急取证**（架构验收件）：单主机深度取证，菜单收窄 host 维度；落地 = 零机制增量。

### 13.4 候选业务清单（路线图位，入选理由 = 离了轮次循环做不成）

| 业务 | 理由 | 预期增量 |
|---|---|---|
| 紫队闭环（票 81） | 攻击 fixture→假设源→自主发现率；发现判定走 ground truth 断言 | eval 资产级 |
| 钓鱼邮件调查 | 扇出宽度由"URL 命中后查出 N 个收件人"现场决定，N 事先不可知 | 工具级（2-3 新工具 + PII 联动） |
| 告警风暴归因 | 时序重建 kill chain，每轮补查目标由上轮发现命名 | 模板级 |

### 13.4b 能力菜单工具位清单（票 70 产物④，票 71 对账输入）

**共享底座**（现有件，零新增）：investigation 六工具（get_alert / siem_query / related_alerts / kb_verify / add_timeline_entry / add_task_log）。
**新增维度工具**（票 78 落）：file_change_query / outbound_conn_query / web_access_query / proc_lineage_query。
**weknora 三工具**（票 79 Memory stub → 票 83 换真）：playbook_lookup / graph_query / hypothesis_register（L1 写，人审口径对齐 INV-5）。

| 业务 | 菜单子集 |
|---|---|
| 威胁狩猎（首发） | 共享底座 + 四新维度工具 + weknora 三工具 |
| 应急取证（验收件） | 共享底座 + proc_lineage / file_change / outbound_conn + weknora 三工具（host 维度收窄） |
| 紫队闭环 | = 狩猎族复用（fixture→假设映射表为唯一新增，票 81） |
| 钓鱼邮件（路线图） | web_access / outbound_conn / siem_query / related_alerts + 未来 2-3 工具（宽度扇出特性） |
| 告警归因（路线图） | related_alerts / siem_query / web_access / file_change / kb_verify |

任何业务新增都不得超出"工具级"粒度（§13.1 分层铁律）。

### 13.5 逼问结论登记（11 维 + Agent 加问 + Web 加问，逐维关闭）

| 维度 | 结论 | 状态 |
|---|---|---|
| ① 边界与用户 | 新场景 S7 主动狩猎（假设→验证/证伪→沉淀），成功标准=收敛结论可查可溯；**五不做**：不改旧链动态化 / 循环不自动执行 L2 / 不并发多假设 / planner 不发明菜单外动作 / weknora 未就绪用 stub 顶 | 适用 [已确认 2026-09-12] |
| ② 业务对象 | 假设 = M2 第七实体（状态机见语义核心）；judge 分岔落点：命中建 Case（挂 hypothesis_id），未命中落 note 型 TimelineEntry + hypothesis_register 写"假设-证据-证伪"关系（人审后进检索面）；轮次/子 run 为 run 内部结构，不进实体表 | 适用 [已确认 2026-09-12] |
| ③ 功能与规则 | 假设的操作清单：发起（POST /hypotheses）/取消（发起人，hunting 态）/列表/详情；新增 INV-11（子票 scope ⊆ 父菜单）；planner/judge/gap 行为规则归 spec（票 72） | 适用 [已确认 2026-09-12] |
| ④ 跨对象联动 | 子 run 终态→事件唤醒父 run；round k outcome→outbox 拉起 round k+1；judge 命中→复用建案路径（Case 挂 hypothesis_id）；未命中→TimelineEntry + 图谱关系 | 适用 [AI自动补全，阶段 2 复核] |
| ⑤ 流程与状态机 | 假设五态：proposed/hunting/concluded/refuted/cancelled；cancelled 四因（人取消/超时/预算耗尽/系统故障），发起人可重新发起新假设；终态不回退（INV-10 覆盖） | 适用 [已确认 2026-09-12] |
| ⑥ 查询与指标 | 自主发现率（紫队，票 81 口径：预算内 ground truth 断言）+ 平均每假设轮数/token/耗时（成本 CSV 同口径续上） | 适用 [已确认 2026-09-12] |
| ⑦ 角色权限 | 发起：soc1 + duty_lead；模板维护：admin；红队不可发起；fixture→假设为系统行为（actor=system） | 适用 [已确认 2026-09-12] |
| ⑧ 外部集成 | weknora 三工具先 Memory stub（票 79）后 HTTP 换真（票 83，WEKNORA_URL 开关）；weknora 侧零安全闸，包层即闸门 | 适用 [已确认 2026-09-12] |
| ⑨ 异常与边界 | 取消=子任务安全停+审计；预算耗尽/故障→cancelled（不冒充 refuted，fail-closed 口径）；weknora 挂掉=查询 fail-closed 降级（INV-1）；防转=相邻轮同组合指纹即停 | 适用 [已确认 2026-09-12] |
| ⑩ 量级与数字 | 单轮 ≤2 子任务、max_rounds=20、防转 max_repeat=1（模板可按业务改写）；**预算按 kind 分档**——hunt_flow 的 run 级/轮级数字独立于旧链（旧链 60s/20 步/50k 不适用于 20 轮循环），具体档位票 72 spec 定并回测压测四天花板 | 适用 [已确认 2026-09-12] |
| ⑪ 数据生命周期 | 假设记录（含证伪）只增不减、不可删，终态即归档；图谱侧剧本由 admin 维护 | 适用 [已确认 2026-09-12] |
| Agent 加问 | 行动边界/人在回路/输出校验/可追溯/可审计 → §13.2 水位线覆盖，阶段 2 复核 | 适用 [AI自动补全] |
| Web 加问 | 独立狩猎页（见 §13.7 页面清单与逐页数据需求） | 适用 [已确认 2026-09-12] |

### 13.6 待决项（B 类，逼问循环）

零。[2026-09-12 全部拍板清零，进入收口对账]

### 13.7 页面清单与逐页数据需求（Web 加问产出）

**页面清单**（每页一行：页面名 / 用途 / 归属模块）：既有六页不动（告警列表 m1+m2 / 流水线 m3 / 审批 m2+m9 / 案件时间线 m2+m8 / 审计流 m2 / Eval 结果 m11）；**新增第七页：狩猎页 m14**——假设入口表单 + 轮次实时视图。

**狩猎页数据需求逐条**：
| 需求 | 类型 | 覆盖状态 |
|---|---|---|
| 假设列表（状态 Tag：五态） | 读 | 需要新查询面：GET /hypotheses（§13.5 ③） |
| 发起假设（选模板 + 填假设句） | 写 | 需要新接口：POST /hypotheses |
| 取消假设（hunting 态按钮） | 写 | 同上资源 PATCH/取消端点 |
| 轮次视图：每轮组合 C_k + 子 run 状态 | 读+实时推送 | 需要新查询面 + SSE：run 公开查询面已有（流水线页同款），轮次子 run 归集需新读接口 |
| judge 裁决 + gap 缺口描述 | 读+实时推送 | 走假设详情读接口（新查询面） |
| 收敛结论（命中 Case 链接 / 未命中证伪摘要） | 读 | 假设详情读接口内嵌（Case 链接复用现有案件查询面） |

"需要新采集"条目为零——全部消费 §13.5 已定实体与 run 事件面；两条"新查询面"（假设 CRUD 读面 + 轮次归集读面）为票 71 页面映射节落卡对象。
...
## 附：冻结后范围变更记录（v1.0 → v1.1，2026-09-04）
...
7. **新增压测工具面 m13**（2026-09-12，用户拍板"按 sdd-flow 做压力测试"）：`scripts/bench/`（宿主侧非运行时件，不进 compose 拓扑）用 autocannon 实测四个理论天花板（分发循环 ~10 run/s / autorun 2s 消费 / SQLite 单写者水位线 / SSE 扇出预算）并找压测拐点；被测侧 `@fastify/under-pressure` 挂 `UNDER_PRESSURE=on` env 才装配（默认形态逐字节不变）。范围 in：单端点微基准/链路突发与持续流/循环水位爬坡/SSE 扇出/防线压下实验（fail-closed 断言）；范围 out：分布式发压、生产 SLO、真 key 压 LLM、官方三件套调优。设计源：`docs/research/2026-09-12-压力测试方案.md`；拆票 64-69。不动 §12 六条冻结决策。

---

## 附：冻结后范围变更记录（v1.1 → v1.2 逼问中，2026-09-12，票 70）

- **触发**：新增「假设驱动编排循环」能力（动态多 LLM 节点编排 + 五业务复用规划）。事实底座：目标场景 缺口叙事（`立项调研.md（已脱敏）`）+ 票 70 前讨论拍板。
- **形式**：PRD 翻「逼问中」，仅受影响维度重开；新增 §13 章为变更唯一落点，冻结内容不动。
- **待拍板条目**：全部登记 §13.6，清零后翻「已定稿」并同步票 71。
